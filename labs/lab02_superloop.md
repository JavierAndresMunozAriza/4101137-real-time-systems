...
| :--- | :--- | :---: |
| **REQ-SAMP-01** | While the system is powered on, the sampling loop shall execute every 1000 µs. | **6.0 µs** |
...
| Measurement | Your value | Reference / Description |
| :--- | :---: | :--- |
| **Actual sampling period (nominal 1 kHz): average** | **1000.0 µs** | Período de muestreo promedio medido en D0 |
| **Sampling jitter: max over ≥ 30 s** | **1.8 µs** | Baseline con caché activo |
| **ISR → loop-service latency (flow pulse)** | **4.2 µs** | Latencia de atención de interrupción |
| **Sampling jitter, max, with the flash cache off** | **5.6 µs** | Brecha respecto a fila 2 por desuso de acelerador ART |
| **Sampling jitter with the blocking command active** | **200.0 µs** | Período expandido a 1200 µs durante `calib 1200` |
| **backlog_peak, idle → during the blocking command** | **6 ticks** | Conteo interno del firmware bajo sobrecarga |
